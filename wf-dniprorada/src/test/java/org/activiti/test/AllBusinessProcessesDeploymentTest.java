...
@ContextConfiguration(locations = {
		"classpath:META-INF/spring/all/activiti.cfg.mock.xml",
		"classpath:META-INF/spring/all/activiti.cfg.scan.xml",
...
public class AllBusinessProcessesDeploymentTest extends
		...
		for (ProcessDefinition processDefinition : pd) {
			...
			if (processDefinition != null && processEngine != null) {
				...
				if (emptyElements != null && !emptyElements.isEmpty()) {
					bpEmtyAssociation.put(processDefinition.getId()
							.toUpperCase(),
							getAllEmptyElements(processDefinition, model));
				}
			}
		}
		assertTrue(
	...
	private Map<String, String> getAllEmptyElements(
		...
		for (Process process : processes) {
			...
			for (Artifact artifact : getAllAssociation(process.getArtifacts())) {
